...
LOCAL_EMBEDDING_MODEL = "all-MiniLM-L6-v2"  # Alternative local model
...
class VecinitaLoader:
    ...
    def __init__(self):
        ...
        if USE_LOCAL_EMBEDDINGS and LOCAL_EMBEDDINGS_AVAILABLE:
        ...
        elif OPENAI_AVAILABLE and not USE_LOCAL_EMBEDDINGS:
            ...
            logger.warning("No embedding model available. Chunks will be loaded without embeddings.")
            ...
            for line in f:
                ...
                if chunk_start_match:
                    ...
                    if not current_source_url:
                        logger.warning(f"Found chunk at line {line_count} but no source URL was set. Skipping.")
                        current_chunk = None
    ...
    def load_file(self, file_path: str, batch_size: int = BATCH_SIZE) -> Dict[str, int]:
        ...
        queue_response = self.supabase.table('processing_queue').insert(queue_entry).execute()
    ...
    def verify_installation(self) -> bool:
        ...
        try:
            ...
            response = self.supabase.table('document_chunks').select('id').limit(1).execute()
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Load scraped content into Vecinita vector database')
    parser.add_argument('input', help='Input file or directory path')
    parser.add_argument('--batch-size', type=int, default=BATCH_SIZE, help='Batch size for processing')
    parser.add_argument('--pattern', default='*.txt', help='File pattern for directory loading')
    parser.add_argument('--verify-only', action='store_true', help='Only verify database installation')
    ...
    if os.path.isfile(args.input):
        ...
        # Print summary
        total_successful = sum(s.get('successful', 0) for s in all_stats.values() if 'successful' in s)
